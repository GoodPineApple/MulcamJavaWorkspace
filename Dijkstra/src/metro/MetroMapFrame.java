...
import metro.MetroMapFrame.MyMouseListener;
...
public class MetroMapFrame extends JFrame {
	...
	class DialogCalcResult extends JDialog {
		...
		JLabel departLabel = new JLabel(depart); /// 출발역의 VO.Stn 이 들어와야됨
		JLabel arriveLabel = new JLabel(arrive); /// 도착역의 VO.Stn 이 들어와야됨
		...
		DialogCalcResult(ActionListener actionListener) {
			...
			comboBox.setModel(new DefaultComboBoxModel(new String[] {"09:04", "2", "3", "4", "5"})); 
			comboBox.setBounds(80, 142, 29, 22);
			...
			JLabel text2 = new JLabel("     <출발 시각> ");
			JLabel textSpace1 = new JLabel("                  "); //여백
			JLabel textSpace2 = new JLabel("                  "); //여백
			JLabel textSpace3 = new JLabel("                                  "); //여백
			
			// 출발시간 검색했을 때 어떤 결과가 나올지 더 논의가 필요함.
			JLabel text4 = new JLabel("출발역 :");
			JLabel text5 = new JLabel("도착역 :");
			JLabel text6 = new JLabel("환승정보 :");
			
			//
			JLabel duration = new JLabel(routecal.getDistance() + " 분"); ////////////// d안에 소요시간
													////////////// 입력!!!

			// 왼쪽페널과 오른쪽페널 위치, 크기, 색깔
			panelLeft.setBackground(Color.white);
			panelLeft.setLocation(20, 80);
			panelLeft.setSize(90, 400);
			panelRight.setBackground(Color.white);
			panelRight.setLocation(150, 80);
			panelRight.setSize(130, 400);

			// 왼쪽페널 붙이기입니다. add하는대로 차례대로 아래로 붙는 것이 목표입니다.
			// add순서 : 출발역 이미지 - 출발역 라벨 - 화살표1 - 환승역1 이미지 - 환승역1 이름 - 화살표2 - 도착역
			// 이미지 - 도착역 라벨

			// 중첩 if문을 상당히 활용해야할 것 같습니다.
			// ex) 환승의 횟수에 따라 if 혹은 switch 문을 실행해야하고, 그 내부에서 lineNum에 따라 다른 이미지를
			// 붙여야한다.
			// 만약 환승역이 없을경우 출발역과 도착역만 실행한다. 만약 환승역이 하나일경우 출발역 환승역 도착역을 실행한다. 만약
			// 환승역이 둘이라면 출발역 환승역1, 환승역2, 도착역을 실행한다.
			// 만약 출발역의 라인넘버가 1호선 이라면 1호선 이미지 불러내고, 출발역의 Stn라벨 쓰고... 만약 출발역의
			// 라인넘버가 2호선이라면 2호선 이미지불러내고...
			
//			panelLeft.add(line1Label);
//			panelLeft.add(departLabel);
//			panelLeft.add(arrowLabel1);
//			panelLeft.add(line2Label);
//			panelLeft.add(transfer1Label);
...
//			panelLeft.add(arriveLabel);
			...
			text2.setFont(new Font("돋움", Font.BOLD, 15));
			...
			btn.addActionListener(new ActionListener() {
				@Override
				public void actionPerformed(ActionEvent e) {
					String departTime = (String) comboBox.getSelectedItem(); // 이거 가져가서 쓰셔야되요...

					System.out.println(departTime);

					panelRight.add(text4); // 출발역 :
					text4.setFont(new Font("돋움", Font.PLAIN, 15));
					panelRight.add(text5); // 도착역 :
					text5.setFont(new Font("돋움", Font.PLAIN, 15));
					panelRight.add(text6); // 환승정보 :
					text6.setFont(new Font("돋움", Font.PLAIN, 15));
				}
			});
