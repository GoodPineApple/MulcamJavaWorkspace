...
public class DijkstraBusan {
	...
	public void calDistance(int startSub, int endSub) { // 최단거리 구하는 메소드
		...
		finalDistance = distance[endSub - 1];
		k = endSub - 1; // k = 종착역
		while (true) {
		...
		System.out.println();
		for (l = 0; l < n; l++) {
			System.out.println(startSub + "에서 " + (l + 1) + "까지의 최단거리는  " + distance[l] + " 입니다.");
		}
	}
...
//		input.close(); // 스캐너 닫기
//		d.getfinalDistance(end);
//		d.getfinalPath(start, end);
//	}
}