...
public class ClientFrame extends JFrame {
	...
	public ClientFrame() {
		...
		try {
			Socket socket = new Socket(InetAddress.getByName("70.12.109.120"), 1234);

			bw = new BufferedWriter(new OutputStreamWriter(socket.getOutputStream()));
			br = new BufferedReader(new InputStreamReader(socket.getInputStream()));
			// 서버와 연결한 후에 닉네임 입력해서 전송하기
			String nickname = JOptionPane.showInputDialog(this, "대화명 입력하세요.", JOptionPane.MESSAGE_PROPERTY);
		...
		public void run() {
			try {
				while (true) {
					...
					chatArea.setCaretPosition(chatArea.getText().length()); //////////요거
				}
			} catch (IOException e) {
				e.printStackTrace();
			}
